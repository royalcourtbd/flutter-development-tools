...
from common_utils import timer_decorator, open_file_with_default_app
    ...
    for root, dirs, files in os.walk(source_folder):
        ...
        for filename in files:
            ...
            try:
                ...
                failed_files += 1
                print(f"❌ Failed to process {rel_file_path}: {e}")
...
def merge_specific_paths_from_file(paths_file, output_file):
    ...
    if not os.path.exists(paths_file):
        print(f"❌ Error: Paths file '{paths_file}' does not exist!")
        return 0, 1
    ...
    try:
        ...
        if not paths:
            print(f"⚠️  Warning: No paths found in '{paths_file}'")
            return 0, 0

        print(f"📁 Processing {len(paths)} paths from {paths_file}")
...
def merge_files():
    ...
    if processed > 0 or failed == 0:
        print(f"✅ Paths file merge completed: {processed} files processed, {failed} failed")
    else:
        print(f"❌ Paths file merge failed or no files found")

    # Append ChatGPT command at the end of the file
    print(f"\n🤖 Adding ChatGPT command to output file...")
    chat_gpt_command = " "
    try:
        with open(path_output_file, "a", encoding="utf-8") as path_outfile:
            path_outfile.write(chat_gpt_command)
        print(f"✅ ChatGPT command added successfully")
    except Exception as e:
        print(f"❌ Error adding ChatGPT command: {e}")
        total_failed += 1
    ...
    if total_failed == 0:
        ...
        print(f"📁 Check your merged file: {path_output_file}")
        # Auto-open the merged file
        open_file_with_default_app(path_output_file)
    else:
        print(f"\n⚠️  COMPLETED WITH SOME ERRORS!")
        print(f"ℹ️  Check the error messages above for details")
